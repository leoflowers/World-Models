#python 06_train_vaegan.py --new_model

from vaegan.arch import VAEGAN # assume VAEGAN looks just like VAE
import argparse
import numpy as np
import config
import os
...
def main(args):
  ...
  if not new_model:
    try:
      ...
      print("Either set --new_model or ensure ./vaegan/weights.h5 exists")
      raise
  ...
  for epoch in range(epochs):
    ...
    vaegan.save_weights('./vaegan/weights.h5')
    vaegan.train(data)
    

if __name__ == "__main__":
  parser = argparse.ArgumentParser(description=('Train VAEGAN'))
